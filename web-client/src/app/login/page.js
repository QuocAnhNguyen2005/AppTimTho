    ...
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--bg-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 16px' }}>
      <div style={{ backgroundColor: 'var(--bg-secondary)', borderRadius: '24px', boxShadow: '0 20px 60px rgba(0,0,0,0.12)', padding: '48px', width: '100%', maxWidth: '440px', border: '1px solid var(--border-color)' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
          {[
            { label: 'Google', color: '#EA4335', shadow: 'rgba(234,67,53,0.1)' },
            { label: 'Zalo', color: '#0068FF', shadow: 'rgba(0,104,255,0.1)' },
            { label: 'Facebook', color: '#1877F2', shadow: 'rgba(24,119,242,0.1)' },
          ].map(s => (
            <button key={s.label}
              style={{ flex: 1, padding: '11px 8px', border: '1.5px solid var(--border-color)', borderRadius: '12px', backgroundColor: 'var(--bg-primary)', cursor: 'pointer', fontSize: '13px', fontWeight: '600', color: 'var(--text-primary)', transition: 'all 0.2s' }}
        ...
        <form onSubmit={handleLogin}>
          {errorMsg && <div style={{ padding: '12px', backgroundColor: '#FEE2E2', color: '#DC2626', borderRadius: '8px', marginBottom: '16px', fontSize: '14px', fontWeight: '500' }}>{errorMsg}</div>}
          {successMsg && <div style={{ padding: '12px', backgroundColor: '#D1FAE5', color: '#059669', borderRadius: '8px', marginBottom: '16px', fontSize: '14px', fontWeight: '500' }}>{successMsg}</div>}
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            <input type="text" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} placeholder="Nhập số điện thoại"
              style={{ width: '100%', padding: '13px 16px', borderRadius: '12px', border: '1.5px solid var(--border-color)', backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '15px', outline: 'none', transition: 'border-color 0.2s', boxSizing: 'border-box' }}
